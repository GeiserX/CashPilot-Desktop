...
- **Services were killed after 20 seconds no matter what they asked for.** Storj declares a 300-second stop timeout because a storage node has to flush before it goes, and being SIGKILLed early costs an unclean-shutdown recovery on the next start. Stop, restart, redeploy and remove now all use the grace period the service's own entry asks for, and a redeploy stops the old container cleanly before replacing it instead of killing it outright.

- **A redeploy could move a node's data and bring it back as a different node.** The replacement container mounted what the catalog says rather than what the running container actually has, so a service whose data lives somewhere the catalog does not know about — set up by hand, or moved outside the app — came back on the catalog's own volume, with a new identity and no reputation. A redeploy now keeps every mount where it already is, unless the user typed a new path for it on this deploy, and says so while it deploys.
